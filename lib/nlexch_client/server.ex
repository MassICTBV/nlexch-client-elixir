...
defmodule NLexchClient.Server do
  use HTTPoison.Base
  use GenServer
  #############################################################################
  ### GenServer Callback
  ...
  defp process_url(url) do
    host = Application.get_env(:nlexch_client, :host) || System.get_env("HOST") || "https://app.nlexch.com"
    host <> url
  end
  ...
  def account_name(account) do
    String.to_atom "#{account}.api.nlexch.com"
  end
  ...
  # REF: https://app.nlexch.com/documents/api_v2#!/members/GET_version_members_me_format
